...
exports.addCompetenceToGroup = asyncHandler(async (req, res, next) => {
  ...
  if (currentCompetenceList.includes(req.body.competenceId)){
    return next(new ErrorResponse(`Competence of id: ${req.body.competenceId} is already in group of id: ${req.params.groupid}`, 400));
  }
...
exports.deleteCompetenceFromGroup = asyncHandler(async (req, res, next) => {
  ...
  currentCompetenceList = competenceGroup.competenceListId;
  if (!currentCompetenceList.includes(req.body.competenceId)){
    return next(new ErrorResponse(`Competence of id: ${req.body.competenceId} dont exists in group of id: ${req.params.groupid}`, 400));
  }
...
exports.getGroupedCompetencesCompareWithUserRating = asyncHandler(async (req, res, next) => {
  const competenceGroups = await GroupCompetence.find({workplaceId: req.user.workplace}).populate('competenceListId');
