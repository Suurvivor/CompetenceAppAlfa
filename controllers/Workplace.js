...
exports.getWorkplacesByDepartment = asyncHandler(async (req, res, next) => {
   ...
   if (!workplace[0]) {
      return next(
         new ErrorResponse(`Could not find any workplace in Department of id ${req.params.departmentid}`, 400)
      );
   }
...
exports.getWorkplace = asyncHandler(async (req, res, next) => {
   const workplace = await Workplace.findById(req.params.id).populate('department');
   if (!workplace) {
      return next(new ErrorResponse(`Could not find workplace of id ${req.params.id}`, 400));
   }
   res.status(200).json({
